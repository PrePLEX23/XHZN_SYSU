...
void dlio::MapNode::savePCD(std::shared_ptr<direct_lidar_inertial_odometry::srv::SavePCD::Request> req,
 ...
 /**
   ...
   */
  void dlio::MapNode::saveJpgYamlPcd(pcl::PointCloud<PointType>::Ptr pc_map, double req_resolution,std::string file_pure_path) {
    ...
    std::cout << "intensity range is " << min_intensity << " --> " << max_intensity
         << std::endl;
    // o 转换色系, 得到渲染后的图像
