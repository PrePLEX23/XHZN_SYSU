...
void dlio::OdomNode::getParams() {
  ...
  // Version
  dlio::declare_param(this, "~dlio/version", this->version_, "0.0.0");

  // Frames
  dlio::declare_param(this, "~dlio/frames/odom", this->odom_frame, "map");
  dlio::declare_param(this, "~dlio/frames/baselink", this->baselink_frame, "base_link");
  dlio::declare_param(this, "~dlio/frames/lidar", this->lidar_frame, "laser_link");
  dlio::declare_param(this, "~dlio/frames/imu", this->imu_frame, "imu_link");

  // Deskew Flag
  dlio::declare_param(this, "~dlio/pointcloud/deskew", this->deskew_, true);

  // Gravity
  dlio::declare_param(this, "~dlio/odom/gravity", this->gravity_, 9.80665);

  // Keyframe Threshold
  dlio::declare_param(this, "~dlio/odom/keyframe/threshD", this->keyframe_thresh_dist_, 0.5);
  dlio::declare_param(this, "~dlio/odom/keyframe/threshR", this->keyframe_thresh_rot_, 5.0);

  // Submap
  dlio::declare_param(this, "~dlio/odom/submap/keyframe/knn", this->submap_knn_, 10);
  dlio::declare_param(this, "~dlio/odom/submap/keyframe/kcv", this->submap_kcv_, 10);
  dlio::declare_param(this, "~dlio/odom/submap/keyframe/kcc", this->submap_kcc_, 10);

  // Dense map resolution
  dlio::declare_param(this, "~dlio/map/dense/filtered", this->densemap_filtered_, true);

  // Wait until movement to publish map
  dlio::declare_param(this, "~dlio/map/waitUntilMove", this->wait_until_move_, false);

  // Crop Box Filter
  dlio::declare_param(this, "~dlio/odom/preprocessing/cropBoxFilter/size", this->crop_size_, 1.0);

  // Voxel Grid Filter
  dlio::declare_param(this, "~dlio/pointcloud/voxelize", this->vf_use_, true);
  dlio::declare_param(this, "~dlio/odom/preprocessing/voxelFilter/res", this->vf_res_, 0.1);

  // Adaptive Parameters
  dlio::declare_param(this, "~dlio/adaptive", this->adaptive_params_, true);

  // Extrinsics
  std::vector<double> t_default1{0.9, 0., 0.85};
  std::vector<double> R_default{1., 0., 0., 0., 1., 0., 0., 0., 1.};

  // center of gravity to imu
  std::vector<double> baselink2imu_t, baselink2imu_R;
  dlio::declare_param(this, "~dlio/extrinsics/baselink2imu/t", baselink2imu_t, t_default1);
  dlio::declare_param(this, "~dlio/extrinsics/baselink2imu/R", baselink2imu_R, R_default);
  ...
  std::vector<double> t_default2{0.96, 0., 1.95};
  std::vector<double> baselink2lidar_t, baselink2lidar_R;
  dlio::declare_param(this, "~dlio/extrinsics/baselink2lidar/t", baselink2lidar_t, t_default2);
  dlio::declare_param(this, "~dlio/extrinsics/baselink2lidar/R", baselink2lidar_R, R_default);

  // this->extrinsics.baselink2lidar.t =
  //   Eigen::Vector3f(baselink2lidar_t[0], baselink2lidar_t[1], baselink2lidar_t[2]);
  this->extrinsics.baselink2lidar.t =
    Eigen::Vector3f(t_default2[0], t_default2[1], t_default2[2]);
  ...
  // IMU
  dlio::declare_param(this, "~dlio/odom/imu/calibration/accel", this->calibrate_accel_, true);
  dlio::declare_param(this, "~dlio/odom/imu/calibration/gyro", this->calibrate_gyro_, true);
  dlio::declare_param(this, "~dlio/odom/imu/calibration/time", this->imu_calib_time_, 3.0);
  dlio::declare_param(this, "~dlio/odom/imu/bufferSize", this->imu_buffer_size_, 2000);
  ...
  dlio::declare_param(this, "~dlio/odom/imu/approximateGravity", this->gravity_align_, true);
  dlio::declare_param(this, "~dlio/imu/calibration", this->imu_calibrate_, true);
  dlio::declare_param(this, "~dlio/imu/intrinsics/accel/bias", prior_accel_bias, accel_default);
  dlio::declare_param(this, "~dlio/imu/intrinsics/gyro/bias", prior_gyro_bias, gyro_default);

  // scale-misalignment matrix
  std::vector<double> imu_sm_default{1., 0., 0., 0., 1., 0., 0., 0., 1.};
  std::vector<double> imu_sm;

  dlio::declare_param(this, "~dlio/imu/intrinsics/accel/sm", imu_sm, imu_sm_default);
  ...
  // GICP
  dlio::declare_param(this, "~dlio/odom/gicp/minNumPoints", this->gicp_min_num_points_, 64);
  dlio::declare_param(this, "~dlio/odom/gicp/kCorrespondences", this->gicp_k_correspondences_, 20);
  dlio::declare_param(this, "~dlio/odom/gicp/maxCorrespondenceDistance", this->gicp_max_corr_dist_,
      std::sqrt(std::numeric_limits<double>::max()) );
  dlio::declare_param(this, "~dlio/odom/gicp/maxIterations", this->gicp_max_iter_, 48);
  dlio::declare_param(this, "~dlio/odom/gicp/transformationEpsilon", this->gicp_transformation_ep_, 0.0005);
  dlio::declare_param(this, "~dlio/odom/gicp/rotationEpsilon", this->gicp_rotation_ep_, 0.0005);
  dlio::declare_param(this, "~dlio/odom/gicp/initLambdaFactor", this->gicp_init_lambda_factor_, 1e-9);

  // Geometric Observer
  dlio::declare_param(this, "~dlio/odom/geo/Kp", this->geo_Kp_, 0.8);
  dlio::declare_param(this, "~dlio/odom/geo/Kv", this->geo_Kv_, 1.2);
...
void dlio::OdomNode::getScanFromROS(const sensor_msgs::msg::PointCloud2::SharedPtr& pc) {
  ...
  static int count = 0;
  std::stringstream ss;
  ss << "/home/ros-monitor/dlio_lscar_ws/orig_scan/" << count++ << ".pcd";
  pcl::io::savePCDFileBinary(ss.str(), *original_scan_);
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  double sweep_ref_time = static_cast<double>(rclcpp::Time(this->scan_header_stamp).seconds());
  //   std::cout << "rclcpp::Time(this->scan_header_stamp).seconds() * 10e9:" << rclcpp::Time(this->scan_header_stamp).seconds() * 10e9 << std::endl;
  // std::cout << "rclcpp::Time(this->scan_header_stamp).seconds():" << rclcpp::Time(this->scan_header_stamp).seconds() << std::endl;

  // std::cout << "sweep_ref_time:" << sweep_ref_time << std::endl;
  ...
  if (this->sensor == dlio::SensorType::OUSTER) {
    ...
    extract_point_time = [&sweep_ref_time](boost::range::index_value<PointType&, long> pt)
      { 
        // std::cout << "pt.value().time:"  << pt.value().time << std::endl;
        return sweep_ref_time + pt.value().time ; 
      
        // std::cout << "sweep_ref_time - pt.value().time * 10e6:" << sweep_ref_time - pt.value().time * 10e6 << std::endl;
  ...
  // std::cout << "this->scan_stamp1:" << this->scan_stamp << std::endl;
  // std::cout << "this->imu_buffer.back().stamp2:" << this->imu_buffer.back().stamp  << std::endl;
  // don't process scans until IMU data is present
  if (!this->first_valid_scan) {
    // std::cout << "this->imu_buffer.back().stamp2:" << this->imu_buffer.back().stamp / 1000000000 << std::endl;
    //   std::cout << "this->scan_stamp1:" << this->scan_stamp << std::endl;
    if (this->imu_buffer.empty() || this->scan_stamp <= (this->imu_buffer.back().stamp)) {
      return;
    }
  ...
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> frames;
  // std::cout << "this->prev_scan_stamp:" << this->prev_scan_stamp << std::endl;
  frames = this->integrateImu(this->prev_scan_stamp, this->lidarPose.q, this->lidarPose.p,
...
#pragma omp parallel for num_threads(this->num_threads_)
  ...
  static int index = 0;
  std::stringstream ss;
  ss << "/home/ros-monitor/dlio_lscar_ws/desk_scan/" << index++ << ".pcd";
  pcl::io::savePCDFileBinary(ss.str(), *this->deskewed_scan);
...
void dlio::OdomNode::callbackImu(const sensor_msgs::msg::Imu::SharedPtr imu_raw) {
  ...
  double imu_stamp_secs = static_cast<double>(rclcpp::Time(imu->header.stamp).seconds());
  ...
  if (this->first_imu_stamp == 0.) {
    ...
    std::cout << "imu_stamp_secs:" << imu_stamp_secs << std::endl;
  }
